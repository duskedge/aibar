...
> **一条教训。** 开发 M4 时，我们翻了 `~/.grok/sessions/` 和 Grok CLI 二进制的
> 端点表，没找到额度，就下结论「Grok 没有官方额度」，还写进了文档和 UI。
> 实际只是找错了地方——上游自己的界面明明在显示这个数。
>
> **「我没找到」不等于「它不存在」。** 把它写成产品结论之前，
> 先去看上游自己的界面显示了什么，那是最强的存在性证据。
